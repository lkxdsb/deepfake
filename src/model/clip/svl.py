...
class SynoDecoder(nn.Module):
    def __init__(
        ...
        # lightweight temporal context head to capture frame-to-frame inconsistency
        self.temporal_hidden = max(d_model // 8, 64)
        branch_width = max(d_model // 16, 32)
        ...
        fused_dim = (
            self.feat_t_dim +
            self.temporal_hidden * 2 +  # bidirectional GRU output
            branch_width * len(self.temporal_pool_branches)
        )
        self.temporal_fusion = nn.Linear(fused_dim, self.feat_t_dim)
    ...
    def _temporal_contextualize(self, frame_tokens, y_t):
        # frame_tokens: (b, t, width)
        _, h_n = self.temporal_gru(frame_tokens)
        gru_feat = h_n.transpose(0, 1).reshape(frame_tokens.size(0), -1)

        multi_scale_feats = []
        temporal_tokens = frame_tokens.transpose(1, 2)  # (b, width, t)
        ...
        fused = torch.cat(
            [
                y_t,
                gru_feat,
                torch.cat(multi_scale_feats, dim=-1)
            ],
            dim=-1
        )
    ...
    def forward(self, x):
        ...
        if OpMode.T in self.op_mode:
            cls_tokens = x[:, :, 0, :]
            y_t = self._temporal_contextualize(cls_tokens, y_t)

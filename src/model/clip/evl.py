...
class CrossComponentSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        self.cls_norm = LayerNorm(feature_dim)
        self.token_norm = LayerNorm(feature_dim)

        self.cls_to_tokens = Attention(
            q_in_dim=feature_dim,
            k_in_dim=feature_dim,
            v_in_dim=feature_dim,
            qk_proj_dim=qkv_dim,
            v_proj_dim=qkv_dim,
            num_heads=num_heads,
            out_dim=feature_dim,
        )
        self.tokens_to_cls = Attention(
            q_in_dim=feature_dim,
            k_in_dim=feature_dim,
            v_in_dim=feature_dim,
            qk_proj_dim=qkv_dim,
    ...
    def forward(self, x: torch.Tensor):
        ...
        cls_tokens = x[:, :, :1, :]
        patch_tokens = x[:, :, 1:, :]
        P = patch_tokens.size(2)

        cls_flat = cls_tokens.squeeze(2)
        tokens_flat = patch_tokens.reshape(N, T * P, C)

        cls_norm = self.cls_norm(cls_flat)
        tokens_norm = self.token_norm(tokens_flat)

        cls_context = self.cls_to_tokens(cls_norm, tokens_norm, tokens_norm)
        tokens_context = self.tokens_to_cls(tokens_norm, cls_norm, cls_norm)

        cls_tokens = cls_tokens + self.dropout(cls_context.unsqueeze(2))
        patch_tokens = patch_tokens + self.dropout(tokens_context.reshape(N, T, P, C))
...
class EVLDecoder(nn.Module):
    ...
    def forward(self, in_features: List[Dict[str, torch.Tensor]]):
        ...
        for i in range(self.num_layers):
            ...
            if self.enable_ccsa:
                frame_features = self.ccsa[i](frame_features)

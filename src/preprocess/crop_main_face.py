...
def get_video_frame_data(fdata_path):
    ...
    for landmarks in frame_landmarks:
        ...
        for lm in landmarks:
            lm_arr = np.array(lm)
            if lm_arr.shape[0] == 98:
                lm_arr = lm_arr[_98_to_68_mapping]
            frame_lms.append(lm_arr)
        processed_landmarks.append(frame_lms)
    frame_landmarks = processed_landmarks
    ...
    for bboxes in frame_bboxes:
        ...
        for bbox in bboxes:
            ...
            if bbox_arr.ndim == 1:
                bbox_arr = bbox_arr.reshape((2, 2))
            frame_bboxes_arr.append(bbox_arr)
        processed_bboxes.append(frame_bboxes_arr)
    frame_bboxes = processed_bboxes

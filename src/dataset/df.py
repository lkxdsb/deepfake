...
class DF(DeepFakeDataset):
    """DeepFakes dataset class for FF++ manipulation method"""
    TYPE_DIRS = {
        'REAL': 'real/',
        'DF': 'DF/',
    }
    ...
    def __init__(
        ...
        **kargs
        ...
        assert self.split == "test", "DF only supports the test set."
        # configurations
        self.df_types = list(DF.TYPE_DIRS.keys())
...
class DFDataModule(DeepFakeDataModule):
    def __init__(
        self,
        compressions: List[str] = [],
        *args, **kargs
        ...
        self.compressions = sorted(
            set([i for i in compressions if i in DF.COMPRESSIONS]),
            reverse=True
        )

    def prepare_data(self):
        DF.prepare_data(self.data_dir, self.compressions, self.vid_ext)
...
if __name__ == "__main__":
    ...
    save_folder = f"./misc/extern/dump_dataset/df/test/"

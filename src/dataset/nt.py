...
class NT(DeepFakeDataset):
    """NeuralTextures dataset class for FF++ manipulation method"""
    TYPE_DIRS = {
        'REAL': 'real/',
        'NT': 'NT/',
    }
    ...
    def __init__(
        ...
        **kargs
        ...
        assert self.split == "test", "NT only supports the test set."
        # configurations
        self.df_types = list(NT.TYPE_DIRS.keys())
...
class NTDataModule(DeepFakeDataModule):
    def __init__(
        self,
        compressions: List[str] = [],
        *args, **kargs
        ...
        self.compressions = sorted(
            set([i for i in compressions if i in NT.COMPRESSIONS]),
            reverse=True
        )

    def prepare_data(self):
        NT.prepare_data(self.data_dir, self.compressions, self.vid_ext)
...
if __name__ == "__main__":
    ...
    save_folder = f"./misc/extern/dump_dataset/nt/test/"

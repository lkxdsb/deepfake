...
class FS(DeepFakeDataset):
    """FaceSwap dataset class for FF++ manipulation method"""
    TYPE_DIRS = {
        'REAL': 'real/',
        'FS': 'FS/',
    }
    ...
    def __init__(
        ...
        **kargs
        ...
        assert self.split == "test", "FS only supports the test set."
        # configurations
        self.df_types = list(FS.TYPE_DIRS.keys())
...
class FSDataModule(DeepFakeDataModule):
    def __init__(
        self,
        compressions: List[str] = [],
        *args, **kargs
        ...
        self.compressions = sorted(
            set([i for i in compressions if i in FS.COMPRESSIONS]),
            reverse=True
        )

    def prepare_data(self):
        FS.prepare_data(self.data_dir, self.compressions, self.vid_ext)
...
if __name__ == "__main__":
    ...
    save_folder = f"./misc/extern/dump_dataset/fs/test/"

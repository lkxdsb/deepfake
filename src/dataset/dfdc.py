...
class DFDC(DeepFakeDataset):
    ...
    def _build_video_list(self):
        ...
        for index, row in video_table.iterrows():
            ...
            # try to resolve the file name in case the csv contains folder prefixes
            # (e.g., "FAKE/rubsrklwhx") while the videos are stored flat under
            # `videos/`. We keep a list of candidates and pick the first that exists
            # in the metadata table.
            candidates = [name, path.basename(name)]
            if "/" in name:
                parts = name.split("/", 1)
                if parts[0] in ("REAL", "FAKE"):
                    candidates.append(parts[1])

            resolved_name = next((n for n in candidates if n in self.video_table), None)

            if resolved_name:
                clips = int(self.video_table[resolved_name]["duration"] // self.clip_duration)
                if (clips > 0):
                    clips = min(clips, self.max_clips)
                    label_videos[label].append((label, resolved_name, clips))
            else:
                logging.warning(
...
class DFDCDataModule(DeepFakeDataModule):
    ...
    def prepare_data(self):
        DFDC.prepare_data(self.data_dir, self.vid_ext)
...
if __name__ == "__main__":
    ...
    # save_folder = f"./misc/extern/dump_dataset/dfdc/test/"
    # # entity dump

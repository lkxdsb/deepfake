...
class F2F(DeepFakeDataset):
    """Face2Face dataset class for FF++ manipulation method"""
    TYPE_DIRS = {
        'REAL': 'real/',
        'F2F': 'F2F/',
    }
    ...
    def __init__(
        ...
        **kargs
        ...
        assert self.split == "test", "F2F only supports the test set."
        # configurations
        self.df_types = list(F2F.TYPE_DIRS.keys())
        ...
        for df_type in self.df_types:
            ...
            for comp in self.compressions:
                ...
                for idx in video_metas:
                    video_metas[idx]["path"] = path.join(self.data_dir, video_metas[idx]["path"].lstrip('/')) + self.vid_ext
...
class F2FDataModule(DeepFakeDataModule):
    def __init__(
        self,
        compressions: List[str] = [],
        *args, **kargs
        ...
        self.compressions = sorted(
            set([i for i in compressions if i in F2F.COMPRESSIONS]),
            reverse=True
        )

    def prepare_data(self):
        F2F.prepare_data(self.data_dir, self.compressions, self.vid_ext)
...
if __name__ == "__main__":
    ...
    save_folder = f"./misc/extern/dump_dataset/f2f/test/"

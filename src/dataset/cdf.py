    ...
    def _build_video_table(self):
        ...
        for df_type in self.TYPE_DIRS:
            ...
            for idx in video_metas:
                # cached path may carry a leading slash; strip it before joining to data_dir
                rel_path = video_metas[idx]["path"].lstrip("/\\")
                video_metas[idx]["path"] = path.join(self.data_dir, rel_path) + self.vid_ext

...
def inference_driver(cli, cfg_dir, ckpt_path, notes=None):
    ...
    for dts_name, dataloader in test_dataloaders.items():
        ...
        # 单卡推理无需分布式收集，分布式时再做 all_gather
        # Guard against accidental calls to distributed APIs when not initialized.
        ...
        if torch.distributed.is_available():
            try:
                if torch.distributed.is_initialized():
                    ...
                    distributed_ready = world_size > 1
            except ValueError:
                distributed_ready = False
        ...
        if distributed_ready:
            ...
            results_to_process = []
            for _batch_results in gathered_results:
                results_to_process.extend(_batch_results)
        else:
            results_to_process = batch_results

        if (not distributed_ready) or (trainer.is_global_zero):
            # fetch predict results and aggregate.
            for batch_result in results_to_process:
